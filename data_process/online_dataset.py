...
class online_data():
    def __init__(self, path="dataset/", datasetname="News", file_path_list=None, test_list=None, validation_list=None,
    ...
    def get_offline(self, do_test=True, do_val=True):
        ...
        if do_val:
            ...
            for i in range(self.start_time - len(
                    self.val_list)):  # some file in real train used be validation, left as train
...
class Streaming_data():
    ...
    def next_train(self, d_time):
        ...
        train transfer save_model,so loss should be compute.
        :return:
         set:      D_t
         set_tt:   D_(t+1)
...
def select_neg_forinteraction(path='dataset/', datasetname='News', file_path_list=None, leave_for_init_train=0.7,
                              neg_num=999):
    inter_all = []
    ...
    not_select_timenum = round(
        all_time * leave_for_init_train)  # [0,1,...,not_select_timenum-1] not need select neg sample
