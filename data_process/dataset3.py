...
class online_amazon(object):
    def __init__(self, stage_num=60):
       ...
       # self.generate_streaming(stage_num, self.score_data)
        ...
        self.cold_users_inter = pd.DataFrame()
        # # # # #generate each time(stage) support and query set
        for i in tqdm(range(59,stage_num)):
          self.generate_current_suppAndQuery(i, types="only_new")
    ...
    def gen_attribution(self, user_data, item_data):#'categories', 'title', 'price', 'sales_type', 'sales_rank', 'brand'
    ...
    def split_warm_cold(self, rat_data, max_count=10):
        ...
        for u_id in user_ids:
            ...
            if u_count>5 and 5<self.user_current_interNum[u_id] <= 20:
                user_cold_list.append(u_id)
                self.cold_users.append(u_id)
                self.cold_users_inter = pd.concat([self.cold_users_inter, u_info]).reset_index(drop=True)
        ...
        print('num warm users: %d, num cold users: %d' % (len(user_warm_list), len(user_cold_list)))
        print('min count: %d, avg count: %d, max count: %d, sum count: %d' % (min(user_counts), len(rat_data) / n_user_ids, max(user_counts),sum(user_counts)))
...
class online_yelp(object):
    def __init__(self, stage_num=40):
        ...
        self.user_dict, self.item_dict = self.generate(self.path, self.item_data,self.user_data)
        # warm_users, cold_users, warm_users_data, cold_users_data = self.split_warm_cold(self.score_data)
...
#user_id:token	user_review_count:float	yelping_since:float	user_useful:float	user_funny:float	user_cool:float	fans:float	average_stars:float	compliment_hot:float	compliment_more:float	compliment_profile:float	compliment_cute:float	compliment_list:float	compliment_note:float	compliment_plain:float	compliment_cool:float	compliment_funny:float	compliment_writer:float	compliment_photos:float
        ...
        user_data=user_data.drop(columns=['user_name','elite','yelping_since','user_useful', 'user_funny', 'user_cool','compliment_note','compliment_plain','compliment_cool','compliment_funny'])

        print("num of user attributions:",user_data.user_review_count.nunique(),user_data.fans.nunique(),user_data.average_stars.nunique(),user_data.compliment_hot.nunique(),user_data.compliment_more.nunique(),user_data.compliment_profile.nunique(),user_data.compliment_cute.nunique(),user_data.compliment_list.nunique(),user_data.compliment_writer.nunique(),user_data.compliment_photos.nunique())
        ...
        print("item attribution num:",item_data.city.nunique(),item_data.state.nunique(),item_data.postal_code.nunique(),item_data.item_stars.nunique(),item_data.item_review_count.nunique())
    ...
    # 10
    def user_converting(self, row, u_count_list, fans_list, average_stars_list, c_hot_list,c_more_list,c_profile_list,c_cute_list,c_list_list,c_writer_list,c_photos_list):
        # gender_dim: 2, age_dim: 7, occupation: 21
        ...
        return [count_idx, fans_idx, average_stars_idx, c_hot_idx,c_more_idx,c_profile_idx,c_cute_idx,c_list_idx,c_writer_idx,c_photos_idx]
    #6
    def item_converting(self, row, city_list, state_list, post_list, cate_list,i_stars_list,i_count_list):#,is_open_list):
        # rate_dim: 6, year_dim: 1,  genre_dim:25, director_dim: 2186,
        city_idx = city_list.index(str(row['city']))
        state_idx = state_list.index(str(row['state']))
        post_idx = post_list.index(str(row['postal_code']))
        i_star_idx = i_stars_list.index(str(row['item_stars']))
        count_idx = i_count_list.index(str(row['item_review_count']))
        ...
        for cate in str(row['categories']).split(", "):
            idx = cate_list.index(re.sub(r'\([^()]*\)', '', cate))
            cate_idx[idx] = 1
    ...
    def generate(self, master_path, item_data, user_data):
        ...
        if not os.path.exists("{}/item_dict.pkl".format(master_path)):
            ...
            for idx, row in item_data.iterrows():
                if row['item_id'] == 'item_id': continue  # 跳过表头
                i_info = self.item_converting(row, city_list, state_list, post_list, cate_list,i_stars_list,i_count_list)#,is_open_list)
        ...
        if not os.path.exists("{}/user_dict.pkl".format(master_path)):
            ...
            for idx, row in user_data.iterrows():
                if row['user_id'] == 'user_id' or row['gender'] == 'Gender': continue  # 跳过表头
                u_info = self.user_converting(row, u_count_list, fans_list, average_stars_list, c_hot_list,c_more_list,c_profile_list,c_cute_list,c_list_list,c_writer_list,c_photos_list)
                user_dict[int(row['user_id'])] = u_info
